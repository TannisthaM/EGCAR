...
validate_egcar_experiment_config <- function(config) {
  ...
  # Backward compatibility for configuration objects created by egcar <= 0.2.4
  # or by older standalone scripts left in the R global environment.  These
  # fields control retention only; supplying their current memory-first defaults
  # does not alter estimators, grids, folds, seeds, scoring, or tuning.
